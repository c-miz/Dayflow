...
void ScheduleModel::saveToJson() const
{
    ...
    if(file.open(QIODevice::WriteOnly)) {
        ...
        qWarning() << "无法保存日程数据:" << file.errorString();
    }
}

void ScheduleModel::loadFromJson()
{
    beginResetModel();
    items.clear();
    ...
    if(file.open(QIODevice::ReadOnly)) {
        ...
        foreach(const QJsonValue &value, jsonArray) {
            items.append(ScheduleItem::fromJson(value.toObject()));
        }
    }
...
QString ScheduleModel::getStoragePath() const
{
